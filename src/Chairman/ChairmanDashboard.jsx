    ...
    <>
      <div className="flex h-screen bg-gradient-to-br from-blue-50 via-indigo-50 to-green-50 overflow-hidden">
        ...
        <div className="lg:hidden fixed top-4 left-4 z-50">
          <button
            ...
            className="p-3 bg-white rounded-xl shadow-lg text-purple-700 hover:bg-purple-50 transition-all"
          >
        ...
        <aside className={`
          fixed lg:static inset-y-0 left-0 z-40 w-72 bg-gradient-to-b from-purple-800 via-purple-700 to-purple-900 text-white 
          transform transition-transform duration-300 ease-in-out flex flex-col shadow-2xl
          ${sidebarOpen ? 'translate-x-0' : '-translate-x-full lg:translate-x-0'}
        `}>
          <div className="p-6 border-b border-purple-600 flex-shrink-0 flex items-center justify-between">
            <div className="flex items-center gap-3">
              <div className="p-2 bg-yellow-400 rounded-xl">
                <FaStamp className="text-purple-900 text-xl" />
              </div>
              <div>
          ...
          <nav className="flex-1 overflow-y-auto p-4 space-y-2">
            {menuItems.map((item) => {
              ...
              return (
                ...
                >
                  <Icon className={`text-lg ${isActive ? 'text-purple-600' : 'text-purple-200'}`} />
          ...
          <div className="p-5 border-t border-purple-600 bg-purple-800 flex-shrink-0">
            <div className="flex items-center gap-4">
              <div className="w-14 h-14 bg-yellow-400 rounded-full flex items-center justify-center text-purple-900 font-bold text-xl shadow-lg">
                AB
              </div>
          ...
          <header className="bg-white shadow-md p-6 border-b border-gray-200 sticky top-0 z-20 flex-shrink-0">
            <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4">
              <div>
                ...
                <p className="text-purple-600 mt-1 text-sm sm:text-base">Web-Based Kebele Management System • Group 4</p>
              </div>
              <div className="text-right">
                <p className="text-sm text-gray-600">
                  {currentTime.toLocaleDateString('en-GB', { day: 'numeric', month: 'long', year: 'numeric' })}
                </p>
                <p className="text-lg font-semibold text-purple-700 flex items-center justify-end gap-2">
      ...
      <style jsx>{`
        @media print {
          ...
          .text-purple-600 { color: #000 !important; }
        }
      `}</style>
    </>
  );
